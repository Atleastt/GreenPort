    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-6">Formulir Edit Kriteria</h3>
                ...
                <form action="{{ route('admin.kriteria.update', $kriteria) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <input type="text" name="nama_kriteria" id="nama_kriteria" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:border-gray-600 dark:text-gray-300" value="{{ old('nama_kriteria', $kriteria->nama_kriteria) }}" required>
                        </div>
                        <div>
                            <label for="deskripsi_kriteria" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi</label>
                            <textarea name="deskripsi_kriteria" id="deskripsi_kriteria" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:border-gray-600 dark:text-gray-300" required>{{ old('deskripsi_kriteria', $kriteria->deskripsi_kriteria) }}</textarea>
                        </div>
                    </div>
